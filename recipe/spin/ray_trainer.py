...
class RayPPOTrainer(object):
    ...
    def fit_dpo(self): # Renamed for clarity as standard PPO loop
        ...
        # --- FIXED TypeError Handling ---
        self.global_steps = loaded_step + 1 if loaded_step is not None and loaded_step > 0 else 1
        # --- End Fix ---
        print(f"Starting PPO training from global step {self.global_steps}. Total steps: {self.total_training_steps}")


        # Perform validation before training
        if self.val_reward_fn is not None and self.config.trainer.get('val_before_train', True):
            print("Running validation before PPO training...")
            val_metrics = self._validate()
            pprint(f'Initial validation metrics: {val_metrics}')
            if logger and val_metrics: logger.log(data=val_metrics, step=max(0, self.global_steps - 1))
            if self.config.trainer.get('val_only', False):
                print("Validation only mode enabled. Exiting PPO training.")
                if logger and hasattr(logger, 'finish'): logger.finish()
                return

        # Add tqdm progress bar
        progress_bar = tqdm(total=self.total_training_steps, initial=self.global_steps, desc="PPO Training Progress", position=0, leave=True)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            print(f"--- Starting PPO Epoch {epoch} ---")
            try:
            ...
            for batch_idx, batch_dict in enumerate(train_iterator):
                ...
                try: # Outer try-except for the whole step
                    ...
                    with _timer('step', timing_raw):
                        ...
                        print(f"\n[Step {self.global_steps}, PPO Batch {batch_idx}] Processing batch size: {current_batch_size}")
                        ...
                        # Keep original non-tensor data if needed later
                        ...
                        # +++ DEBUG: Print info about batch being sent to generate_sequences +++
                        print(f"---- [Step {self.global_steps}] DEBUG PPO: Sending to generate_sequences ----")
                        print(f"Batch Keys: {list(gen_batch.batch.keys())}")
                        print(f"Non-Tensor Keys: {list(gen_batch.non_tensor_batch.keys())}") # Should be empty now
                        print(f"Meta Info: {gen_batch.meta_info}") # Check meta info being passed
                        for key, tensor in gen_batch.batch.items():
                             print(f"  Input Tensor '{key}':")
                             ...
                             if tensor.numel() > 0:
                                 try:
                                     ...
                                     # Basic checks (removed vocab check to avoid previous error)
                                     if 'input_ids' in key and min_val < 0: print(f"    ERROR: Negative input_id!")
                                     elif 'attention_mask' in key and not torch.all((tensor == 0) | (tensor == 1)): print(f"    ERROR: attention_mask has values other than 0 or 1!")
                                     elif 'position_ids' in key:
                                         if min_val < 0: print(f"    ERROR: Negative position_id!")
                                         if max_val >= tensor.shape[1]: print(f"    ERROR: position_id {max_val} >= seq len {tensor.shape[1]}!")
                                 except Exception as e_check: print(f"    ERROR checking min/max for {key}: {e_check}")
                             else: print("    Tensor is empty!")
                        print("---------------------------------------------------------------")
                        # +++ End DEBUG +++
                        ...
                        with _timer('gen', timing_raw):
                            try:
                                ...
                                # +++ DEBUG: Print info about generation output +++
                                print(f"---- [Step {self.global_steps}] DEBUG PPO: Generation Output Received ----")
                                print(f"Output Batch Keys: {list(gen_batch_output.batch.keys())}")
                                print(f"Output Non-Tensor Keys: {list(gen_batch_output.non_tensor_batch.keys())}")
                                print(f"Output Meta Info: {gen_batch_output.meta_info}")
                                for key, tensor in gen_batch_output.batch.items():
                                     ...
                                     if tensor.numel() > 0:
                                        ...
                                        except Exception: print("    Min/Max check failed.")
                                     else: print("    Tensor is empty!")
                                print("-----------------------------------------------------------------")
                                
                                
                                # print the two response and the prompt
                                
                                
                                # +++ End DEBUG +++
                            except Exception as gen_e:
                                print(f"\n!!!!!!!! ERROR DURING PPO GENERATION (Step {self.global_steps}) !!!!!!!!")
                                print(gen_e)
                                traceback.print_exc()
                                print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                                step_timer.stop(); continue # Skip rest of step if generation fails
                        
                        
                        # --- Start of PPO Specific Processing ---
                        # Restore original non-tensor data to the batch before union
                        batch.non_tensor_batch = original_non_tensor_data


                        batch.non_tensor_batch['uid'] = np.array([str(uuid.uuid4()) for _ in range(current_batch_size)], dtype=object) # Adjust if batch size changed due to repeat
                        # Repeat original prompt data *before* union if needed by subsequent steps
                        batch = batch.repeat(repeat_times=self.config.actor_rollout_ref.rollout.n, interleave=True)
                        # Combine with generated sequences
                        batch = batch.union(gen_batch_output)
                        

                        # --- Add checks after union ---
                        print(f"---- [Step {self.global_steps}] DEBUG PPO: After Union ----")
                        print(f"Batch Keys: {list(batch.batch.keys())}")
                        print(f"Non-Tensor Keys: {list(batch.non_tensor_batch.keys())}")
                        print(f"Meta Info: {batch.meta_info}")
                        print(f"Batch Size: {batch.batch.batch_size}")
                        print("-------------------------------------------------------")
                        ...
                        # --- End Add checks ---
                         ...
                         # --- Print Decoded Sample (Handles n=2) ---
                        try:
                            # Get first prompt (index 0 of the repeated batch)
                            prompt_ids_sample = batch.batch['prompts'][0]
                            prompt_mask_sample = batch.batch['attention_mask'][0][:len(prompt_ids_sample)] # Mask for prompt part
                            prompt_len_sample = prompt_mask_sample.sum().item()
                            prompt_text_sample = self.tokenizer.decode(prompt_ids_sample[:prompt_len_sample], skip_special_tokens=True)

                            # Get the two responses for the first prompt (indices 0 and 1 of the repeated batch)
                            response1_ids_sample = batch.batch['responses'][0]
                            response2_ids_sample = batch.batch['responses'][1]
                            response1_text_sample = self.tokenizer.decode(response1_ids_sample, skip_special_tokens=True)
                            response2_text_sample = self.tokenizer.decode(response2_ids_sample, skip_special_tokens=True)

                            print(f"\n---- [Step {self.global_steps}] DEBUG PPO: Decoded Sample 0 (n=2) ----")
                            print(f"Prompt:\n{prompt_text_sample}")
                            print("-" * 20)
                            print(f"Response 1:\n{response1_text_sample}")
                            print("-" * 20)
                            print(f"Response 2:\n{response2_text_sample}")
                            print("------------------------------------------------------\n")

                        except Exception as decode_e:
                            print(f"ERROR during debug decoding/printing: {decode_e}")
                        # --- End Print Decoded Sample ---

                        
                        batch.batch['response_mask'] = compute_response_mask(batch)
                        print(f"---- [Step {self.global_steps}] DEBUG PPO: response_mask ----")
                        print(f"  Shape: {batch.batch['response_mask'].shape}, DType: {batch.batch['response_mask'].dtype}, Min: {batch.batch['response_mask'].min().item()}, Max: {batch.batch['response_mask'].max().item()}")
                        print("-------------------------------------------------------")

                        if self.config.trainer.balance_batch:
                            print(f"---- [Step {self.global_steps}] DEBUG PPO: Before _balance_batch ----")
                            self._balance_batch(batch, metrics=metrics)
                            print(f"---- [Step {self.global_steps}] DEBUG PPO: After _balance_batch ----")
                            # Optional: Print shapes again if balancing might change things (unlikely for shapes)
                            # print(f"  Balanced batch input_ids shape: {batch.batch['input_ids'].shape}")


                        batch.meta_info['global_token_num'] = torch.sum(batch.batch['attention_mask'], dim=-1).tolist()
                        print(f"---- [Step {self.global_steps}] DEBUG PPO: global_token_num (sample) ----")
                        print(f"  {batch.meta_info['global_token_num'][:5]}...") # Print first 5
                        print("------------------------------------------------------------")
                        ...
                        with _timer('old_log_prob', timing_raw):
                            ...
                            print(f"---- [Step {self.global_steps}] DEBUG PPO: old_log_probs ----")
                            if 'old_log_probs' in batch.batch:
                                tensor = batch.batch['old_log_probs']
                                print(f"  Shape={tensor.shape}, DType={tensor.dtype}")
                                if tensor.numel() > 0:
                                    try: print(f"  Min={tensor.min().item():.4f}, Max={tensor.max().item():.4f}, Mean={tensor.mean().item():.4f}")
                                    except Exception as e: print(f"  Stat check failed: {e}")
                            else: print("  'old_log_probs' key missing!")
                            print("---------------------------------------------------------")
                        ...
                        if self.use_reference_policy:
                            with _timer('ref', timing_raw):
                                ...
                                print(f"---- [Step {self.global_steps}] DEBUG PPO: ref_log_prob ----")
                                if 'ref_log_prob' in batch.batch:
                                     tensor = batch.batch['ref_log_prob']
                                     print(f"  Shape={tensor.shape}, DType={tensor.dtype}")
                                     if tensor.numel() > 0:
                                         try: print(f"  Min={tensor.min().item():.4f}, Max={tensor.max().item():.4f}, Mean={tensor.mean().item():.4f}")
                                         except Exception as e: print(f"  Stat check failed: {e}")
                                else: print("  'ref_log_prob' key missing!")
                                print("--------------------------------------------------------")
                        ...
                        with _timer('adv', timing_raw):
                            if self.use_rm:
                                print(f"---- [Step {self.global_steps}] DEBUG PPO: Calling compute_rm_score ----")
                                reward_tensor_rm = self.rm_wg.compute_rm_score(batch)
                                batch = batch.union(reward_tensor_rm) # Adds 'rm_scores' key
                                print(f"---- [Step {self.global_steps}] DEBUG PPO: rm_scores ----")
                                if 'rm_scores' in batch.batch:
                                    tensor = batch.batch['rm_scores']
                                    print(f"  Shape={tensor.shape}, DType={tensor.dtype}")
                                    if tensor.numel() > 0:
                                        try: print(f"  Min={tensor.min().item():.4f}, Max={tensor.max().item():.4f}, Mean={tensor.mean().item():.4f}")
                                        except Exception as e: print(f"  Stat check failed: {e}")
                                else: print("  'rm_scores' key missing!")
                                print("---------------------------------------------------")


                            reward_extra_infos_dict: dict[str, list] = {}
                            try:
                                # --- This is the main reward calculation call ---
                                print(f"---- [DEBUG Step {self.global_steps}] Calling self.reward_fn... ----") # Added
                                if self.reward_fn is None: # Added Check
                                     print(f"---- [DEBUG Step {self.global_steps}] ERROR: self.reward_fn is None! ----")
                                     reward_result = {'reward_tensor': torch.zeros_like(batch.batch['response_mask'], dtype=torch.float32)} # Dummy zero tensor
                                else:
                                     reward_result = self.reward_fn(batch, return_dict=True)

                                print(f"---- [DEBUG Step {self.global_steps}] self.reward_fn returned keys: {list(reward_result.keys())} ----") # Added
                                reward_tensor = reward_result['reward_tensor'] # Final combined reward
                                print(f"---- [DEBUG Step {self.global_steps}] Reward Tensor Shape: {reward_tensor.shape}, Dtype: {reward_tensor.dtype} ----") # Added
                                if reward_tensor.numel() > 0: # Added Stats
                                     print(f"---- [DEBUG Step {self.global_steps}] Reward Tensor Stats: Min={reward_tensor.min().item():.4f}, Max={reward_tensor.max().item():.4f}, Mean={reward_tensor.mean().item():.4f} ----")
                                else:
                                     print(f"---- [DEBUG Step {self.global_steps}] Reward Tensor is EMPTY! ----")

                                reward_extra_infos_dict = reward_result.get('reward_extra_info', {})
                                if reward_extra_infos_dict: # Added
                                     print(f"---- [DEBUG Step {self.global_steps}] Reward Extra Info Keys: {list(reward_extra_infos_dict.keys())} ----")

                            except Exception as e:
                                print(f'---- [DEBUG Step {self.global_steps}] Error in reward_fn call: {e}. ----') # Added
                                traceback.print_exc() # Added
                                reward_tensor = torch.zeros_like(batch.batch['response_mask'], dtype=torch.float32) # Dummy zero tensor
                            ...
                            if self.config.algorithm.use_kl_in_reward:
                                print(f"---- [Step {self.global_steps}] DEBUG PPO: Applying KL Penalty ----")
                                batch, kl_metrics = apply_kl_penalty(batch, kl_ctrl=self.kl_ctrl_in_reward, kl_penalty=self.config.algorithm.kl_penalty)
                                metrics.update(kl_metrics)
                                print(f"  KL Metrics: {kl_metrics}")
                                print(f"---- [Step {self.global_steps}] DEBUG PPO: token_level_rewards (After KL) ----")
                            else:
                                batch.batch['token_level_rewards'] = batch.batch['token_level_scores']
                                print(f"---- [Step {self.global_steps}] DEBUG PPO: token_level_rewards (No KL) ----")

                            if 'token_level_rewards' in batch.batch:
                                tensor = batch.batch['token_level_rewards']
                                print(f"  Shape={tensor.shape}, DType={tensor.dtype}")
                                if tensor.numel() > 0:
                                    try: print(f"  Min={tensor.min().item():.4f}, Max={tensor.max().item():.4f}, Mean={tensor.mean().item():.4f}")
                                    except Exception as e: print(f"  Stat check failed: {e}")
                            else: print("  'token_level_rewards' key missing!")
                            print("---------------------------------------------------------------------")
                        ...
                        batch = compute_onlineDPO_pref(batch)
                        ...
                        with _timer('prepare_dpo_batch', timing_raw):
                            try:
                                if 'preferences' not in batch.batch or batch.batch['preferences'] is None:
                                    raise ValueError("'preferences' key missing or None in batch after compute_onlineDPO_pref.")
                                # Ensure other necessary keys are present from the earlier steps
                                required_keys = ['input_ids', 'attention_mask', 'old_log_probs'] # Base keys
                                if self.use_reference_policy:
                                ...
                                preferences_mask = batch.batch['preferences'] # Shape [batch_size * n], True=Chosen
                                not_preferences_mask = ~preferences_mask

                                # Check shapes before indexing
                                expected_batch_size = batch.batch.batch_size[0]
                                if preferences_mask.shape[0] != expected_batch_size:
                                     raise ValueError(f"Preferences mask shape {preferences_mask.shape} != batch shape {expected_batch_size}")

                                # --- Gather Chosen/Rejected Tensors using the Mask ---
                                ...
                                # Select position_ids if they exist
                                ...
                                # --- Create Labels for DPO Loss ---
                                # WARNING: Using fixed max_prompt_length. Needs actual prompt lengths per sample.
                                # This might lead to incorrect loss calculation if prompts vary significantly.
                                print("WARNING: Creating DPO labels using configured max_prompt_length. This might be incorrect for variable length prompts.")
                                prompt_len = self.config.data.max_prompt_length # Using configured max length
                                ...
                                # Optional: More precise masking using attention_mask length if needed
                                # full_len_c = chosen_attention_mask.sum(dim=1).long()
                                # for i in range(chosen_labels.shape[0]): chosen_labels[i, full_len_c[i]:] = -100
                                # (Similar logic for rejected_labels)
                                ...
                                dpo_tensors = {
                                     ...
                                     # NOTE: We are NOT passing old/ref logprobs here,
                                     # assuming update_actor_dpo uses ActorAsRef internally
                                }
                                ...
                                # --- Prepare Meta Info ---
                                # Assuming ActorAsRef approach in update_actor_dpo
                                dpo_meta = {
                                     ...
                                     'use_reference_policy': False, # Explicitly False for ActorAsRef
                                     'reference_free': True,        # Explicitly True for ActorAsRef
                                     'global_step': self.global_steps
                                }
                                # Create the final DataProto object to send to the actor worker
                                dpo_update_batch_proto = DataProto.from_dict(tensors=dpo_tensors, meta_info=dpo_meta)
                                print(f"---- [Step {self.global_steps}] DEBUG PPO: Prepared DPO Update Batch ----")
                                print(f"  Keys: {list(dpo_update_batch_proto.batch.keys())}")
                                print(f"  Batch Size: {dpo_update_batch_proto.batch.batch_size}") # Should be original batch size
                                ...
                                traceback.print_exc()
                                dpo_update_batch_proto = None # Ensure we skip update on error
                        # --- End Prepare DPO Batch ---

                        # --- End PPO Calculations ---

                        # --- Start Updates ---
                        # if self.use_critic:
                        #     with _timer('update_critic', timing_raw):
                        #         critic_output = self.critic_wg.update_critic(batch)
                        #     if critic_output and 'metrics' in critic_output.meta_info:
                        ...
                        is_last_step = self.global_steps >= self.total_training_steps # Recalculate here just in case
                        if self.val_reward_fn is not None and test_freq > 0 and (is_last_step or self.global_steps % test_freq == 0):
                            print(f"\nRunning PPO validation at step {self.global_steps}...")
                            val_timing_raw = {}
                            with _timer('testing', val_timing_raw):
                                val_metrics: dict = self._validate() # Use the batched _validate
                            if is_last_step: last_val_metrics = val_metrics
                            if val_metrics: # Only update if validation returned metrics
                        ...
                        if save_freq > 0 and ( is_last_step or self.global_steps % save_freq == 0):
                            print(f"\nSaving PPO checkpoint at step {self.global_steps}...")
                    ...
                    # This line requires 'step' key in timing_raw, ensure the 'with _timer' block completes
                    if 'step' in timing_raw:
                         metrics.update(compute_throughout_metrics(batch=batch, timing_raw=timing_raw, n_gpus=n_gpus))
                    else:
                         print(f"Warning: 'step' key missing from timing_raw at step {self.global_steps}. Skipping throughput calculation.")

                    step_timer.stop() # Stop the manual timer
                    metrics['time/step'] = step_timer.last # Use measured step time
                    ...
                    if logger and self.global_steps % log_freq == 0:
                        ...
                        print(f"[Step {self.global_steps} PPO] Logging Step Payload Keys: {list(log_payload.keys())}")
                    ...
                    # Catch errors within the main step loop
                    print(f"\n!!!!!!!! ERROR DURING PPO Step {self.global_steps} !!!!!!!!")
                    print(f"Caught Exception: {step_e}")
                    traceback.print_exc() # Print the full traceback
                    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    step_timer.stop() # Stop timer even on error
                    should_stop = True; break # Stop training on error

                if is_last_step or should_stop:
                    print(f'Stopping PPO training at step {self.global_steps}.')
                    break # Break inner loop

                # Increment step counter only if step completed without outer error
                self.global_steps += 1
            ...
            if should_stop: break # Break outer loop if needed

        # --- Final cleanup and logging ---
        progress_bar.close()
        final_step = max(0, self.global_steps - 1) # Step number where training actually finished
        print(f"PPO Training finished at step {final_step}.")
        # Save final checkpoint if not saved on last step and not validation only
        save_freq = OmegaConf.select(self.config.trainer, "save_freq", default = -1)
        if not self.config.trainer.get('val_only', False) and (save_freq <= 0 or final_step % save_freq != 0) :
            print(f"Saving final PPO checkpoint at step {final_step}...")
            self._save_checkpoint() # Ensure this saves based on the final step state

        # Final validation run if needed
        ...
        if logger and hasattr(logger, 'finish'): logger.finish()
        print("PPO Training Run Complete.")
        
        
    def fit_dpo2(self): # Renamed for clarity as standard PPO loop
        """
                ...
                try: # Outer try-except for the whole step
                    ...
                    with _timer('step', timing_raw):
                        ...
                        with _timer('reward_calc', timing_raw):
                            ...
                            # (Apply KL penalty logic if needed - modifies 'token_level_rewards')
                            # ... if self.config.algorithm.use_kl_in_reward: ...
